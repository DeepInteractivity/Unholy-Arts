...
window.createCaPenetratePussy = function(initiator, targetsList) {
	...
	ca.execute = function() {
		...
		var lustDamage = ((getChar(this.initiator).physique.getValue() * 1 + getChar(this.initiator).resilience.getValue() * 1) / 8);
		getChar(this.targetsList[0]).lust.changeValue(-lustDamage);
		var lustDamage2 = ((getChar(this.targetsList[0]).physique.getValue() * 1 + getChar(this.targetsList[0]).resilience.getValue() * 1) / 16);
...
window.createCaPenetrateAss = function(initiator, targetsList) {
	...
	ca.execute = function() {
		...
		var lustDamage = ((getChar(this.initiator).physique.getValue() * 1 + getChar(this.initiator).resilience.getValue() * 1) / 12);
		getChar(this.targetsList[0]).lust.changeValue(-lustDamage);
...
window.createCaMountDick = function(initiator, targetsList) {
	...
	ca.validRelationalPositions.push(["mountingFaceToFace","mountedFaceToFace"]);
...
window.createCaDoublePenetration = function(initiator, targetsList) {
	...
	ca.execute = function() {
		...
		var lustDamage = ((getChar(this.initiator).physique.getValue() * 1 + getChar(this.initiator).resilience.getValue() * 1) / 5);
		getChar(this.targetsList[0]).lust.changeValue(-lustDamage);
		var lustDamage2 = ((getChar(this.targetsList[0]).agility.getValue() * 1 + getChar(this.targetsList[0]).resilience.getValue() * 1) / 10);
		getChar(this.initiator).lust.changeValue(-lustDamage2);
		var energyDamage = ((getChar(this.initiator).physique.getValue() * 1 + getChar(this.initiator).resilience.getValue() * 1) / 45);
		getChar(this.targetsList[0]).energy.changeValue(-energyDamage);
		results.value += lustDamage;
...
window.createCaVinesHoldArms = function(initiator,targetsList) {
	...
	ca.execute = function() {
		...
		// Willpower damage
		var generalDamage = ((gC(this.initiator).intelligence.getValue() + gC(this.initiator).resilience.getValue() ) / 40);
		...
		results.description += " " + ktn(target) + " received " + textWillpowerDamage(generalDamage) + " and " + textEnergyDamage(generalDamage) + "." + overflowMsg1 + overflowMsg2 + " " + ktn(target) + " is having trouble to keep control. ";
...
window.createCaSlimeHug = function(initiator,targetsList) {
	...
	ca.execute = function() {
		...
		// General damage
		var generalDamage = ((gC(this.initiator).agility.getValue() + gC(this.initiator).resilience.getValue() + gC(this.initiator).will.getValue() ) / 60);
		...
		results.description += " " + ktn(target) + " received " + textLustDamage(generalDamage) + ", " + textWillpowerDamage(generalDamage) + " and "
							 + textEnergyDamage(generalDamage) + "." + overflowMsg1 + overflowMsg2;
		if ( leadDamage > 0 ) {
