...
window.Character = function(name, varName) {
	...
	this.generateTitledName = function() {
		var names = [];
		for ( var cName of this.names ) {
		...
		var titledName = '<span style="color:'+this.nameColor+'" title="' + tooltip + '">' +this.name+'</span>';
		return titledName;
	}
	...
	this.recoverControl = function() {
		if ( this.control < this.maxControl && this.controlRecovery > 0 ) {
			this.control += this.controlRecovery;
			if ( this.control > this.maxControl ) {
				this.control = this.maxControl;
			}
		}
	}
	this.gainControlBack = function() {
		var gainedControl = this.maxControl * 0.6;
		this.control = gainedControl;
		this.lostControlTurns = -1;
		State.variables.sc.otherMessages.push("" + ktn(this.varName) + " recovered " + gainedControl + " control points.");
	}
	this.attackControl = function(damage) {
		this.control -= damage;
		if ( this.control < 0 ) {
			this.control = 0;
		}
	}
	this.depleteControl = function() {
		this.control = 0;
		this.assignLostControlTurns(1);
	}
	this.lostControlTurns = 0;
	this.assignLostControlTurns = function(value) { // Value does nothing. Perhaps it will do something eventually.
		var lostTurns = limitedRandomInt(2) + 1 // 1 ~ 3
		this.lostControlTurns = lostTurns;
		State.variables.sc.otherMessages.push("" + ktn(this.varName) + " has lost control for " + lostTurns + " turn/s.");
	}
	...
	this.textStats = function() {
		...
		for ( var cStat of getStatNamesArray() ) {
			if ( this[cStat].getValue() < this[cStat].value ) {
				text += firstToCap(cStat) + ": " + colorText(this[cStat].getValue().toFixed(1),"red") + " (" + this[cStat].value.toFixed(0) + ")";
			} else if ( this[cStat].getValue() > this[cStat].value ) {
				text += firstToCap(cStat) + ": " + colorText(this[cStat].getValue().toFixed(1),"green") + " (" + this[cStat].value.toFixed(0) + ")";
			} else {
				text += firstToCap(cStat) + ": " + this[cStat].getValue().toFixed(0);
			}
			if ( cStat != "luck" ) {
				text += "\n";
			}
	...
	this.getCharacterUIbarInfo = function() { // Returns a string that generates the character's basic info when displayed on Sugarcube 2
		var string = "<div align='center'>\ " + this.generateTitledName() + " (" + this.getCharScreenButton("Status") + ")\n";
		if ( this.avatar != null ) {
			string += this.avatar() + "\n";
		}
		string += "</div> \ ";
