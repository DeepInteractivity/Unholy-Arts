...
window.Drive = function(initialValue, initialLevel) {
	...
	this.setValues = function(newValue, newLevel) {
		this.value = newValue;
		this.level = newLevel;
	}
	
	this.addPoints = function(points) { // Positive or negative points. It calls the must be leveled function
		this.value += points;
		this.checkLevelChange();
	}
	this.checkLevelChange = function() {
		if ( this.level > 0 ) {
			if ( this.value < formulaDriveLevelPoints(this.level) ) {
				this.level--;
			} else if ( this.value > formulaDriveLevelPoints(this.level + 1) ) {
				this.level++;
			}
		} else {
			if ( this.value > formulaDriveLevelPoints(this.level + 1) ) {
				this.level++;
			}
		}
	}
