...
window.getAskAboutValues = function(type) {
	...
	socInt.topic = "none";
	socInt.weight = 0; // Starting weight
	...
	socInt.getWeight = function(sis,actor,target,observers,extraData) {
		var nWeight = this.weight + gC(actor).mood.friendly * 0.03 + gC(actor).mood.intimate * 0.05 - gC(actor).mood.angry * 0.10 - gC(actor).mood.bored * 0.05;
		return this.weight;
	}
	...
	socInt.calculateExtraEffect = function() {
		...
		if ( failureVars > successVars ) {
			this.getDescription = function() {
				var desc = (ktn(this.target) + " does not wish to discuss " + gC(this.target).posPr + " values. " + gC(this.target).perPr + " may reconsider it if " + gC(this.target).perPr + " was in a better mood.");
...
window.getComplimentSocInt = function(type) {
	...
	socInt.getDescription = function() {
		var desc = randomFromList( [ (ktn(this.actor) + " only has great things to say about " + ktn(this.target) + "'s " + this.topic + ".") ,
								     (ktn(this.actor) + " complimeted " + ktn(this.target) + "'s " + this.topic + ".") ,
	...
	switch(type) {
		...
		socInt.getDescription = function() {
			var desc = randomFromList( [ (ktn(this.actor) + " caressed " + ktn(this.target) + "'s ear.") ,
										 (ktn(this.actor) + " gently grabed " + ktn(this.target) + "'s ear.") ] );
			return desc;
		}
			break;
	}
