...
window.createCandidateGlobalAi = function(charKey) {
	...
	globalAi.generateSocializationChoices = function() {
		...
		if ( State.variables.simCycPar.templeDayPeriod != "training" ) {
			mChoice.weight += 5;
			mChoice.weight += gC(this.charKey).dPleasure.level * 4 + gC(this.charKey).dLove.level * 4 + gC(this.charKey).dCooperation.level * 4;
		}
	...
	globalAi.generateCombatChoices = function() {
		...
		if ( gSettings().challengingAllowed || gSettings().assaultingAllowed ) {
			...
			if ( selectedTarget != "" ) {
				...
				// WEIGHT WEIGHTS
				if ( State.variables.simCycPar.templeDayPeriod != "training" ) {
					mChoice1.weight += 10 + gC(this.charKey).dAmbition.level * 4 + gC(this.charKey).dImprovement.level * 2;
					mChoice2.weight += 10 + gC(this.charKey).dAmbition.level * 2 + gC(this.charKey).dDomination.level * 4;
				}
...
window.cTrainPhysiqueMissionChoice = function(character) {
	...
	mChoice.weight = 5 + gC(character).dImprovement.level * 2 + gC(character).dAmbition.level;
	return mChoice;
}
